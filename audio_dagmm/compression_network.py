...
class DeconvBlock(nn.Module):
    def __init__(self, in_ch, out_ch, kernel=4, stride=2, pad=1, final=False):
        ...
        if not final:
            layers += [nn.BatchNorm2d(out_ch), nn.LeakyReLU(0.01, inplace=True)] # 0.2 prev
        else:
            layers += [nn.Sigmoid()]
...
class CompressionNetwork(nn.Module):
    def __init__(self, latent_dim: int):
        ...
        self.enc1 = ConvBlock(1,  32)   # 32×32×32
        self.enc2 = ConvBlock(32, 64)   # 64×16×16
        self.enc3 = ConvBlock(64,128)   # 128×8×8
        self.enc4 = ConvBlock(128,256)  # 256×4×4
        # self.adapt   = nn.AdaptiveAvgPool2d((4,4))  # -> [B,256,4,4]
        # dummy = torch.zeros(1,1,64,313)
        # w_enc = self.enc4(self.enc3(self.enc2(self.enc1(dummy)))).shape[-1]
        # self.unpool = nn.Upsample((4, w_enc), mode='bilinear', align_corners=False)
        self.flatten    = nn.Flatten()
        self.fc_latent  = nn.Linear(256*4*4, latent_dim)

        # Decoder: reverse
        self.fc_expand  = nn.Linear(latent_dim, 256*4*4)
        self.unpool = nn.Upsample((4, 19), mode='bilinear', align_corners=False)
        self.dec4 = DeconvBlock(256,128)   # 128×8×8
        self.dec3 = DeconvBlock(128,64)    # 64×16×16
        self.dec2 = DeconvBlock(64,32)     # 32×32×32
        self.dec1 = DeconvBlock(32, 1, final=True)  # 3×64×64
    ...
    def decode(self, z):
        ...
        x = self.dec2(x)
        x = self.dec1(x)
        x = F.interpolate(x, size=(64, 313), mode='bilinear', align_corners=False)
        return x[..., :313]
